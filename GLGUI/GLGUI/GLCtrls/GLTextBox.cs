...
namespace GLGUI
{
    public class GLTextBox : GLCtrl
    {
        ...
        public GLTextBox(GLCtrlContainer container)
            : base(container)
        {
            Render += OnRender;
            ...
            ContextMenu = new GLContextMenu(container);
            ContextMenu.Add(new GLContextMenuEntry(container) { Text = "Copy" }).Click += (s, e) => { if (selectionStart.Index != cursorPosition.Index) CopySelection(); };
            ContextMenu.Add(new GLContextMenuEntry(container) { Text = "Paste" }).Click += (s, e) => { if (Clipboard.ContainsText()) Insert(Clipboard.GetText()); };
        }
